...
import { getSidebarCss, getSidebarHtml } from './sidebar-template.js';
...
export class ResumeHubSidebar {
  ...
  _startObservers() {
    // URL changes are handled via shared locationchange + controller onNavigate.
    // Keep a single debounced DOM observer for content loads within the same URL
    // (job panel text arriving after navigation), not for URL detection.
    let debounceTimeout;
    const scheduleUpdate = () => {
      clearTimeout(debounceTimeout);
      debounceTimeout = setTimeout(() => this._updateJobContext(), 500);
    };
    ...
    window.addEventListener('locationchange', this._locationListener);
    window.addEventListener('popstate', this._locationListener);

    const target =
      document.querySelector('.jobs-search__job-details, .job-view-layout, main') ||
      document.body;
  ...
  /**
   * AI insight fetch helpers below are intentionally kept on the sidebar.
   * JobInsightsManager orchestrates caching/batching and calls these methods —
   * they are not dead duplicates.
   */
...
${jobDescription.substring(0, 3000)}`;
        ...
        if (aiDetails && typeof aiDetails === 'object') {
          ...
          if (details.length > 0) {
            companyStatsEl.innerHTML = details.map(d => `<div style="margin: 4px 0;">${Sanitizer.sanitizeHTML(d)}</div>`).join('');
            return;
          }
        }
      }
...
${jobDescription.substring(0, 3000)}`;
      ...
      if (aiRequirements && Array.isArray(aiRequirements) && aiRequirements.length > 0) {
        ...
        requirementsEl.innerHTML = (filtered.length > 0 ? filtered : aiRequirements.slice(0, 7))
          .map(req => `<div class="rh-requirement-item">• ${Sanitizer.sanitizeHTML(String(req).trim())}</div>`)
          .join('');
        return;
      }
  ...
  _extractKeyRequirementsFallback(jobDescription, requirementsEl) {
      ...
      if (requirements.length > 0) {
        requirementsEl.innerHTML = requirements.slice(0, 5).map(req => 
          `<div class="rh-requirement-item">• ${Sanitizer.sanitizeHTML(req)}</div>`
        ).join('');
      } else {
...
${jobDescription.substring(0, 3000)}`;
      ...
      if (aiSkills && Array.isArray(aiSkills) && aiSkills.length > 0) {
        ...
        skillsEl.innerHTML = uniqueSkills.map(skill => 
          `<span class="rh-skill-tag">${Sanitizer.sanitizeHTML(skill)}</span>`
        ).join('');
        return;
      }
...
${jobDescription.substring(0, 3000)}`;
      ...
      if (aiQuestions && Array.isArray(aiQuestions) && aiQuestions.length > 0) {
        const formattedQuestions = aiQuestions.slice(0, 6).map(q => {
          ...
          return `${icon} ${Sanitizer.sanitizeHTML(question)}${starNote}`;
        });
        ...
        if (validResources.length > 0) {
          resourcesEl.innerHTML = validResources.map(res => 
            `<div class="rh-resource-item">
              <span>🔗</span>
              <a href="${Sanitizer.sanitizeHTML(res.url)}" target="_blank" rel="noopener noreferrer" class="rh-resource-link">${Sanitizer.sanitizeHTML(res.name)}</a>
            </div>`
          ).join('');
          return;
        }
  ...
  _setFallbackResources(resourcesEl) {
    if (!resourcesEl) return;
      ...
      resourcesEl.innerHTML = resources.map(res => 
        `<div class="rh-resource-item">
          <span>🔗</span>
          <a href="${Sanitizer.sanitizeHTML(Sanitizer.sanitizeURL(res.url))}" target="_blank" rel="noopener noreferrer" class="rh-resource-link">${Sanitizer.sanitizeHTML(res.name)}</a>
        </div>`
      ).join('');
    }
